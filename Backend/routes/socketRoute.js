...
const {twitterData} = require("../utils/twitterAPI");
const  verifyNews = require("../utils/verifyNews");
...
module.exports = (io, socket, onlineUsers, db) => {
  ...
  socket.on("location", async (data) => {
    try {
      ...
      // const val = await twitterData()
      // .then(data => {
      //   // if data is an array of tweet objects
      //   const tweetText = data[0]?.text || "No tweet text found";
      //   return verifyNews(tweetText);
      // })
      // .then(res => {
      //   socket.emit("twitterAlert", { message: res });
      // })
      // .catch(err => console.error("Error verifying news:", err));
      // const result = await verifyNews(val);
      // console.log("Verified News:", result);
      // if(userData.IsAlert){
